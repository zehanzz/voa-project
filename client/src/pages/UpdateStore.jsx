...
const UpdateStore = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.put(`/store/${targetID}`, inputs);
    ...
    return (
        <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
            <Header category="Page" title="Update Store" />

            <form id="addForm" className="space-y-6" onSubmit={handleSubmit}>
                <div >
                    <label className="block font-medium text-gray-700 mb-2" >Store ID : {targetID}</label>
                    ...
                    <button className="w-full bg-blue-500 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md focus:outline-none focus:shadow-outline"
                        onClick={handleSubmit}>Update Store</button>
                </div>
            </form>
